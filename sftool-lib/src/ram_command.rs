...
// RamCommand trait is now implemented by each specific chip implementation
// in their respective modules (sf32lb52/mod.rs, sf32lb56/mod.rs, sf32lb58/mod.rs)

pub trait DownloadStub {
    fn download_stub(&mut self) -> Result<(), std::io::Error>;
}

impl<T: SifliTool> DownloadStub for T {
    fn download_stub(&mut self) -> Result<(), std::io::Error> {
