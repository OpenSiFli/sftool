...
// Helper functions for SifliDebug implementation - SF32LB56 specific
...
fn send(
    ...
    let data_with_reserved_len = send_data.len();
    let header = create_header(data_with_reserved_len as u16);
...
fn recv(reader: &mut BufReader<Box<dyn Read + Send>>) -> Result<SifliUartResponse, std::io::Error> {
    ...
    // 步骤2: 读取帧头部(长度，时间戳，通道，CRC) - SF32LB56 specific
    ...
    temp.extend_from_slice(&timestamp_bytes);
    ...
    temp.extend_from_slice(&reserved_bytes);
...
fn map_sf32lb56_address(addr: u32) -> u32 {
    let mut l_addr = addr;
...
impl<T: SifliTool> SifliDebug for T {
    ...
    fn debug_read_word32(&mut self, addr: u32) -> Result<u32, std::io::Error> {
        // SF32LB56: Apply address mapping
        let mapped_addr = map_sf32lb56_address(addr);
    ...
    fn debug_write_memory(&mut self, address: u32, data: &[u8]) -> Result<(), std::io::Error> {
        ...
        // SF32LB56: Apply address mapping first
        let mut mapped_address = map_sf32lb56_address(address);
        
        // Then apply the existing mapping logic
            ...
            if block_addr >= addr_usize && block_end <= addr_usize + data.len() {
                ...
                let mut block: [u8; 4] = match resp {
                    SifliUartResponse::MEMRead { data: d } if d.len() == 4 => {
                        // SF32LB56: Convert from big-endian response to little-endian for internal processing
                        let value = u32::from_be_bytes([d[0], d[1], d[2], d[3]]);
                        value.to_le_bytes()
                    }
                    _ => {
                        return Err(std::io::Error::new(
